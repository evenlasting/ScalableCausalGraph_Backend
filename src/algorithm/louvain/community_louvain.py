...
def modularity(partition, graph, weight='weight'):
    ...
    for node in graph:
        com = partition[node]
        in_deg[com] = in_deg.get(com, 0.) + graph.in_degree(node, weight=weight)
        out_deg[com] = out_deg.get(com, 0.) + graph.out_degree(node, weight=weight)
        for neighbor, datas in graph[node].items():
            edge_weight = datas.get(weight, 1)
            if partition[neighbor] == com:
                if neighbor == node: # a edge to the node itself
                    inc[com] = inc.get(com, 0.) + float(edge_weight) # sum of weights in a community
                else:
    ...
    #            (deg.get(com, 0.) / (2. * links)) ** 2 # compared to the classical algorithm, the importance of the weight has been twice
    for com in set(partition.values()):
        res += (inc.get(com, 0.) / links) - \
               (in_deg.get(com, 0.) / links) * (out_deg.get(com, 0.) / links)
    return res
